...
open class PinboardClient(private val token: String) {
  ...
  open fun deletePost(url: String): Mono<Boolean> {
    ...
    val params = ParamUtils.exchangeParameters(token, "url" to url.trim())
    val buildUrl = buildUrl("/posts/delete", params)
    return isDone(this.client.get().uri(buildUrl, params))
  }
