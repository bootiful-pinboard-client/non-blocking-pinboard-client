...
class PinboardClientTest {
  ...
  private val bookmark = Bookmark("http://garfield.com", "description",
      "extended", "hash", "meta", Date(), false, true, arrayOf(this.testTag, this.testTag2))
  ...
  fun getAllBookmarksByTag() {
    setupNewPost()
    val postsByTag = this.reactivePinboardClient.getAllPosts(tag = this.pinboardClientTestTags).take(1)
    StepVerifier
        .create(postsByTag)
        .expectNextCount(1)
        .verifyComplete()
  ...
  fun deletePost() {
    setupNewPost()
    StepVerifier
        .create(this.reactivePinboardClient.getPosts(url = bookmark.href/*, tag = arrayOf(testTag)*/))
        .expectNextMatches { it.posts.size == 1 }
        .verifyComplete()
    log.info("we have one post with href ${bookmark.href}")

    StepVerifier
        .create(this.reactivePinboardClient.deletePost(bookmark.href!!))
        .expectNextMatches { it }
        .verifyComplete()
    log.info("the bookmark should have been deleted by this point.")

    StepVerifier
        .create(this.reactivePinboardClient.getPosts(url = bookmark.href/*, tag = arrayOf(testTag)*/))
        .expectNextMatches { it.posts.isEmpty() }
        .verifyComplete()
    log.info("we should have zero posts left")
  }
